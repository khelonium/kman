...
//require_once("Xmpp_Logging::LOGGING.php");

class Xmpp extends Xmpp_XMLStream {
	var $server;
	var $user;
	var $password;
	var $resource;
	...
	function message_handler($xml) {
		...
		if(!array_key_exists('type',$paytload)) $payload['type'] = 'chat';
		$payload['from'] = $xml->attrs['from'];
		$sub_body = $xml->sub('body');
		$payload['body'] = (is_object($sub_body))?$sub_body->data:null;
//		$payload['body'] = $xml->sub('body')->data;
		$this->log->log("Message: {$payload['body']}", Xmpp_Logging::LOGGING_DEBUG);
	...
	function presence_handler($xml) {
		...
		$this->log->log("Presence: {$payload['from']} [{$payload['show']}] {$payload['status']}", Xmpp_Logging::LOGGING_DEBUG);
	...
	function sasl_failure_handler($xml) {
		$this->log->log("Auth failed!", Xmpp_Logging::LOGGING_ERROR);
		$this->disconnect();
	}
