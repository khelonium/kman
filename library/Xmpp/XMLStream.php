...
class Xmpp_XMLStream {
	...
	public function __construct($host=Null, $port=Null, $log=False, $loglevel=Null, $is_server=False) {
		...
		$this->port = $port;
		$this->setupParser();
		$this->log = new Xmpp_Logging($log, $loglevel);
		$this->log->log("uat");
	}
	...
	function connect($persistent=False, $sendinit=True) {
		...
		if(!$this->socket) {
			$this->log->log("Could not connect.", Xmpp_Logging::LOGGING_ERROR);
	...
	function process() {
		...
		while(!$this->disconnect) {
			...
			if ($updated > 0) {
				...
				$this->log->log("RECV: $buff", Xmpp_Logging::LOGGING_VERBOSE);
				$this->log->log("PARSING", Xmpp_Logging::LOGGING_VERBOSE);
	...
	function processUntil($event, $timeout=-1) {
		...
		while(!$this->disconnected and $this->until[$event_key] and (time() - $start < $timeout or $timeout == -1)) {
			...
			if ($updated > 0) {
				...
				$this->log->log("PARSINGA", Xmpp_Logging::LOGGING_VERBOSE);
				$this->log->log("RECV: $buff", Xmpp_Logging::LOGGING_VERBOSE);
				xml_parse($this->parser, $buff, False);
			}
		}
	...
	function startXML($parser, $name, $attr) {
		...
		$obj = new Xmpp_XMLObj($name, $ns, $attr);
		if($this->xml_depth > 1)
	...
	function endXML($parser, $name) {
		#$this->log->log("Ending $name", Xmpp_Logging::LOGGING_DEBUG);
		#print "$name\n";
		if($this->been_reset) {
			$this->been_reset = False;
			$this->xml_depth = 0;
		...
		if($this->xml_depth == 1) {
			...
			foreach($this->nshandlers as $handler) {
				...
				if($searchxml !== Null and $searchxml->name == $handler[0] and ($searchxml->ns == $handler[1] or (!$handler[1] and $searchxml->ns == $this->default_ns))) {
					...
					$this->log->log("Calling {$handler[2]}", Xmpp_Logging::LOGGING_DEBUG);
...
//					call_user_method($handler[0], $handler[1], $this->xmlobj[2]);
			...
			if(is_array($this->xmlobj)) {
				...
				if(!array_key_exists(0,$this->xmlobj)){
				    $this->xmlobj[0] = new stdClass();
				}
	...
	function doReconnect() {
		if(!$this->is_server) {
			$this->log->log("Reconnecting...", Xmpp_Logging::LOGGING_WARNING);
	...
	function event($name, $payload=Null) {
		$this->log->log("EVENT: $name", Xmpp_Logging::LOGGING_DEBUG);
	...
	function send($msg) {
		...
		$this->log->log("SENT: $msg", Xmpp_Logging::LOGGING_VERBOSE);
